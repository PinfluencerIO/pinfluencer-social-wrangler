...
namespace Pinfluencer.SocialWrangler.Bootstrapping.DevOps.Wrappers
{
    ...
    public static class AwsElasticBeanstalkDeployFacade
    {
        ...
        {
            new TransferUtility( new AmazonS3Client( credentials.Id, credentials.Token,
                    RegionEndpoint.GetBySystemName( credentials.Region ) ) )
                .Upload( $"{deploy.FilePath}{Path.DirectorySeparatorChar}{deploy.File}", deploy.BucketName,
                    $"{deploy.Application}/{deploy.File}" );
        }

        // TODO: validate errors :)) write tests :))))
        private static void DeployApplication( AwsCredentialsDto credentials, AwsDeployDto deploy )
        {
            var client = new AmazonElasticBeanstalkClient( credentials.Id, credentials.Token,
                RegionEndpoint.GetBySystemName( credentials.Region ) );
            client.CreateApplicationVersionAsync(
                new CreateApplicationVersionRequest( deploy.Application, deploy.VersionLabel )
                {
                    SourceBundle = new S3Location
                        { S3Bucket = deploy.BucketName, S3Key = $"{deploy.Application}/{deploy.File}" }
                } ).Wait( );
